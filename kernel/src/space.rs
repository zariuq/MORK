...
impl Space {
    ...
    pub fn query_multi_raw<
        PZ: ZipperProduct,
        F: FnMut(Result<&[u32], (BTreeMap<(u8, u8), ExprEnv>, u8, u8, &[(u8, u8)])>, Expr) -> bool,
    >(
        mut prz: &mut PZ,
        sources: &[ExprEnv],
        mut effect: F,
    ...
    pub fn transform_multi_multi_(
        ...
        let mut template_prefixes: Vec<_> = templates
            .iter()
            .map(|e| unsafe { e.prefix().unwrap_or_else(|x| e.span()).as_ref().unwrap() })
            .collect();
    ...
    pub fn transform_multi_multi_o(
        ...
        let mut template_prefixes: Vec<_> = sinks
            .iter()
            .map(|sink| match sink.request().next().unwrap() {
