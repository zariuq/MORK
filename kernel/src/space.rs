...
const DEFAULT_MORK_WORK_BUFFER_BYTES: usize = 16 * 1024 * 1024;
const DEFAULT_MORK_ZIPPER_BUFFER_BYTES: usize = 4 * 1024 * 1024;
const MIN_MORK_BUFFER_BYTES: usize = 4096;

fn mork_env_buffer_bytes(var: &str) -> Option<usize> {
    std::env::var(var)
        .ok()
        .and_then(|raw| raw.parse::<usize>().ok())
        .filter(|v| *v >= MIN_MORK_BUFFER_BYTES)
}

pub(crate) fn mork_work_buffer_bytes() -> usize {
    mork_env_buffer_bytes("MORK_WORK_BUFFER_BYTES").unwrap_or(DEFAULT_MORK_WORK_BUFFER_BYTES)
}

fn mork_zipper_buffer_bytes() -> usize {
    mork_env_buffer_bytes("MORK_ZIPPER_BUFFER_BYTES").unwrap_or(DEFAULT_MORK_ZIPPER_BUFFER_BYTES)
}

fn mork_parse_buffer_bytes(input_len: usize) -> usize {
    let hinted = input_len
        .saturating_mul(4)
        .saturating_add(MIN_MORK_BUFFER_BYTES);
    mork_env_buffer_bytes("MORK_PARSE_BUFFER_BYTES")
        .unwrap_or(DEFAULT_MORK_WORK_BUFFER_BYTES)
        .max(hinted)
}

fn allocate_work_buffer(bytes: usize) -> Vec<u8> {
    vec![0u8; bytes]
}
...
impl Space {
    ...
    pub fn load_all_sexpr_impl(&mut self, r: &[u8], add: bool) -> Result<usize, String> {
        let mut stack = allocate_work_buffer(mork_parse_buffer_bytes(r.len()));
        let mut it = Context::new(r);
        let mut i = 0;
        let mut parser = ParDataParser::new(&self.sm);
        loop {
    ...
    pub fn load_sexpr_impl(
        ...
        let mut wz = self.btm.write_zipper_at_path(constant_template_prefix);
        let parse_buffer_len = mork_parse_buffer_bytes(r.len());
        let mut buffer: Vec<u8> = allocate_work_buffer(parse_buffer_len);
        let mut stack = allocate_work_buffer(parse_buffer_len);
        let mut it = Context::new(r);
        let mut i = 0;
        let mut parser = ParDataParser::new(&self.sm);
        loop {
    ...
    pub fn dump_sexpr<W: Write>(&self, pattern: Expr, template: Expr, w: &mut W) -> usize {
        ...
        let mut buffer = allocate_work_buffer(mork_work_buffer_bytes());
        let mut pat = vec![
    ...
    pub fn query_multi<F: FnMut(Result<&[u32], BTreeMap<(u8, u8), ExprEnv>>, Expr) -> bool>(
        ...
        prz.reserve_buffers(mork_zipper_buffer_bytes(), 32);

        Self::query_multi_raw(&mut prz, &pat_args[1..], effect)
    }

    pub fn query_multi_with_factor_exprs<
        ...
        BREAK.with_borrow_mut(|a| {
            if unsafe { setjmp(a) == 0 } {
                coreferential_transition(
                    ...
                    &mut |loc| {
                        let primary = Expr {
                            ptr: loc.origin_path().as_ptr().cast_mut(),
                        };
                        let mut factor_exprs = Vec::with_capacity(sources.len());
                        factor_exprs.push(primary);
                        let mut pairs = vec![(sources[0], ExprEnv::new(1, primary))];
                        ...
                        for (&pa, &other_i) in sources[1..].iter().zip(loc.path_indices()) {
                            let factor_expr = Expr {
                                ptr: unsafe { loc.origin_path().as_ptr().cast_mut().add(other_i) },
                            };
                            factor_exprs.push(factor_expr);
                            pairs.push((pa, ExprEnv::new((pairs.len() + 1) as u8, factor_expr)));
                        }
