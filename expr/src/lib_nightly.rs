...
pub fn item_sink<W: std::io::Write>(
    target: &mut W,
) -> impl Coroutine<Result<Tag, &[u8]>, Yield = (), Return = std::io::Result<Infallible>> {
    #[coroutine]
    move |mut i: Result<Tag, &[u8]>| {
        loop {
            match i {
                Ok(Tag::SymbolSize(_)) => {
...
pub fn item_source<'a>(
    e: Expr,
) -> impl Coroutine<(), Yield = Result<Tag, &'a [u8]>, Return = usize> {
    #[coroutine]
    move || {
...
pub fn apply_e<
    'o,
    OS: Coroutine<Result<Tag, &'o [u8]>, Yield = (), Return = std::io::Result<Infallible>>,
>(
    n: u8,
    ...
    loop {
        match std::pin::pin!(&mut src).resume(()) {
            ...
            CoroutineState::Yielded(Ok(Tag::SymbolSize(_))) => unsafe {
...
mod tests {
    use crate::{Expr, Tag, item_sink, item_source, parse};
    use std::ops::*;
